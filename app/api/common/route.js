...
import { getDynamicModel } from '@/lib/models';
import mongoose from 'mongoose';
import { checkNum } from '@/Helper/Helper';
...
export async function POST(req, { params }) {
  ...
  try {
    ...
    // Transform the data
    const transformedData = Object.entries(restData).reduce((acc, [key, value]) => {
  ...
  try {
    ...
    if (searchParamsData) {
      for (const key of searchParamsData?.keys()) {
        if (key !== "mdl") {
          query[key] = checkNum(searchParamsData.get(key)) && (!searchParamsData.get(key)?.includes("/")) ? parseInt(searchParamsData.get(key)) : searchParamsData.get(key);
        }
      }
    }
    ...
    let cacheKey = `HRMS-${model}:${queryString}`;
 
    // Check if the data is in the cache
     let cachedData = (model !=="employeeDataFinal" && model !=="employeeData" && model !=="AQ_Employee_Data") && 
    ...
    // Cache the result
     (model !=="employeeDataFinal" && model !=="employeeData" && model !=="AQ_Employee_Data") && 
